...
class Dagtaskexecute:
    ...
    def create_pythonoperator_task(self, task_id, function, params):
        if params is not None:
            pass#task = PythonOperator(task_id=task_id, python_callable=function, op_kwargs=params, dag=self.dag)
        else:
            pass#task = PythonOperator(task_id=task_id, python_callable=function, dag=self.dag)
        return None
    ...
    def set_conect_based_on_flow(self):
        ...
        for stepname, functiondetails in dag_flow.items():
            ...
            if count_pos == 0:
                if len(params) > 0:
                    self.dag_tasks[task_id] = function#self.create_pythonoperator_task(task_id, function, params)
                    prev_dag = self.dag_tasks[task_id]
                else:
                    self.dag_tasks[task_id] = function#self.create_pythonoperator_task(task_id, function, None)
                    prev_dag = self.dag_tasks[task_id]
            else:
                if len(params) > 0:
                    self.dag_tasks[task_id] = function#self.create_pythonoperator_task(task_id, function, params)
